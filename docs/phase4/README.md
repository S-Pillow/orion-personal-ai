...
- `p4-04a-hermes-audio-gateway.md` — **IMPLEMENTED IN SOURCE CONTROL / NOT APPLIED TO RUNTIME**. A fail-closed compatibility patch exposes bounded authenticated Hermes STT/TTS routes on the already accepted gateway so Push to Talk can use Hermes without a second voice server.
- draft PR #16 — Orion-side Push to Talk foundation. It deliberately fails closed until the Hermes gateway advertises `audio_api: true`.
...
1. review and runtime-qualify P4-04A against the exact accepted Hermes `0.20.6` source identity;
2. live-validate draft PR #16 Push to Talk on native Windows with the JLab microphone;
3. prove spoken input lands in the same persisted Hermes session as typed HUD chat;
4. prove Hermes-owned TTS can speak the normal streamed HUD response without blocking typed fallback;
